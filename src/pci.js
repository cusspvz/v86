...
var /** @const */ PCI_CONFIG_ADDRESS = 0xcf8,
...
export class PCI {
  constructor(cpu) {
    ...
    this.isa_bridge = {
      ...
      pci_space: [
        ...
        0x86,
        0x80, 0x00, 0x70, 0x07, 0x00, 0x00, 0x02, 0x00, 0x00, 0x01, 0x06, 0x00,
  ...
  set_state(state) {
    for (let i = 0; i < 256; i++) {
      ...
      for (let bar_nr = 0; bar_nr < device.pci_bars.length; bar_nr++) {
        let value = space[(0x10 >> 2) + bar_nr]

        if (value & 1) {
          let bar = device.pci_bars[bar_nr]
          let from = bar.original_bar & ~1 & 0xffff
          let to = value & ~1 & 0xffff
  ...
  pci_query() {
    ...
    let bdf = (this.pci_addr[2] << 8) | this.pci_addr[1],
    ...
    let device = this.device_spaces[bdf]
  ...
  pci_write8(address, written) {
    let bdf = (address >> 8) & 0xffff
    let addr = address & 0xff

    let space = new Uint8Array(this.device_spaces[bdf].buffer)
    let device = this.devices[bdf]
  ...
  pci_write16(address, written) {
    dbg_assert((address & 1) === 0)

    let bdf = (address >> 8) & 0xffff
    let addr = address & 0xff

    let space = new Uint16Array(this.device_spaces[bdf].buffer)
    let device = this.devices[bdf]
  ...
  pci_write32(address, written) {
    dbg_assert((address & 3) === 0)

    let bdf = (address >> 8) & 0xffff
    let addr = address & 0xff

    let space = this.device_spaces[bdf]
    let device = this.devices[bdf]
    ...
    if (addr >= 0x10 && addr < 0x28) {
      ...
      if (bar) {
        ...
        if ((written | 3 | (bar.size - 1)) === -1) {
          ...
          if (type === 0) {
            // memory
            let original_bar = bar.original_bar
        ...
        if (type === 1) {
          ...
          let from = space[space_addr] & ~1 & 0xffff
          let to = written & ~1 & 0xffff
          dbg_log(
            'io bar changed from ' +
              h(from >>> 0, 8) +
              ' to ' +
  ...
  set_io_bars(bar, from, to) {
    ...
    let ports = this.io.ports

    for (let i = 0; i < count; i++) {
      let old_entry = ports[from + i]
      ports[from + i] = this.io.create_empty_entry()

      if (
        old_entry.read8 === this.io.empty_port_read8 &&
        ...
        dbg_log(
          'Warning: Bad IO bar: Source not mapped, port=' + h(from + i, 4),
          LOG_PCI
        )
      }

      let entry = bar.entries[i]
      let empty_entry = ports[to + i]
      dbg_assert(entry && empty_entry)

      ports[to + i] = entry

      if (
        empty_entry.read8 === this.io.empty_port_read8 ||
        ...
        dbg_log(
          'Warning: Bad IO bar: Target already mapped, port=' + h(to + i, 4),
          LOG_PCI
        )
      }
    }
  ...
  raise_irq(pci_id) {
    let space = this.device_spaces[pci_id]
    dbg_assert(space)

    let pin = ((space[0x3c >>> 2] >> 8) & 0xff) - 1
    let device = ((pci_id >> 3) - 1) & 0xff
    let parent_pin = (pin + device) & 3
    let irq = this.isa_bridge_space8[0x60 + parent_pin]
  ...
  lower_irq(pci_id) {
    ...
    let pin = (space[0x3c >>> 2] >> 8) & 0xff
    let device = (pci_id >> 3) & 0xff
