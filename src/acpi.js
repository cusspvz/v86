...
export function ACPI(cpu) {
  ...
  let acpi = {
    pci_id: 0x07 << 3,
    pci_space: [
      0x86, 0x80, 0x13, 0x71, 0x07, 0x00, 0x80, 0x02, 0x08, 0x00, 0x80, 0x06,
  ...
  this.pm1_status = 0
  this.pm1_enable = 0
  this.last_timer = this.get_timer(microtick())
  ...
  io.register_read(0xb008, this, undefined, undefined, function () {
    let value = this.get_timer(microtick()) & 0xffffff
...
ACPI.prototype.timer = function (now) {
  let timer = this.get_timer(now)
  let highest_bit_changed = ((timer ^ this.last_timer) & (1 << 23)) !== 0
