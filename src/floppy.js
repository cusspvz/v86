...
export class FloppyController {
  constructor(cpu, fda_image, fdb_image) {
  ...
  port3F5_write(reg_byte) {
    ...
    if (this.bytes_expecting > 0) {
      ...
      if (this.bytes_expecting === 0) {
        if (DEBUG) {
          let log = '3F5 command received: '
          for (let i = 0; i < this.receiving_index; i++)
      ...
      switch (reg_byte) {
        ...
        case 0xc5:
          this.next_command = function (args) {
            this.do_sector(true, args)
          }
          this.bytes_expecting = 8
          break
        case 0xe6:
          this.next_command = function (args) {
            this.do_sector(false, args)
          }
          this.bytes_expecting = 8
          break
        case 0x07:
  ...
  do_sector(is_write, args) {
    let head = args[2],
      ...
      read_offset =
        ((head + this.number_of_heads * cylinder) * this.sectors_per_track +
          sector -
          1) *
        sector_size
    ...
    dbg_log(
      'from ' + h(read_offset) + ' length ' + h(read_count * sector_size),
      LOG_FLOPPY
    )
    ...
    if (is_write) {
      this.dma.do_write(
        ...
        2,
        this.done.bind(this, args, cylinder, head, sector)
      )
    } else {
      this.dma.do_read(
        this.fda_image,
  ...
  done(args, cylinder, head, sector, error) {
    if (error) {
