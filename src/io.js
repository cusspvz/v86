...
export class IO {
  constructor(cpu) {
    ...
    let memory_size = cpu.memory_size[0]
   ...
   */
  register_read(port_addr, device, r8, r16, r32) {
   ...
   */
  register_write(port_addr, device, w8, w16, w32) {
    dbg_assert(typeof port_addr === 'number')
    dbg_assert(typeof device === 'object')
    ...
    if (DEBUG) {
      let fail = function (n) {
        dbg_assert(
          false,
          'Overlapped write' + n + ' ' + h(port_addr) + ' (' + device.name + ')'
        )
      }
   ...
   */
  register_read_consecutive(port_addr, device, r8_1, r8_2, r8_3, r8_4) {
   ...
   */
  register_write_consecutive(port_addr, device, w8_1, w8_2, w8_3, w8_4) {
    dbg_assert(arguments.length === 4 || arguments.length === 6)
  ...
  mmap_read32_shim(addr) {
    let aligned_addr = addr >>> MMAP_BLOCK_BITS
    let fn = this.cpu.memory_map_read8[aligned_addr]
  ...
  mmap_write32_shim(addr, value) {
    let aligned_addr = addr >>> MMAP_BLOCK_BITS
    let fn = this.cpu.memory_map_write8[aligned_addr]
  ...
  port_write8(port_addr, data) {
    let entry = this.ports[port_addr]
  ...
  port_read8(port_addr) {
    ...
    let value = entry.read8.call(entry.device)
    dbg_assert(
      value < 0x100,
  ...
  port_read16(port_addr) {
    ...
    let value = entry.read16.call(entry.device)
    dbg_assert(
      value < 0x10000 && value >= 0,
      '16 bit port returned large value: ' + h(port_addr)
    )
    return value
  }

  port_read32(port_addr) {
    let entry = this.ports[port_addr]
    ...
    let value = entry.read32.call(entry.device)
    dbg_assert((value | 0) === value)
    return value
  }

  get_port_description(addr) {
