...
import { int_log2_byte } from './utils/int_log'

/** @const */
export const PIC_LOG_VERBOSE = false
...
export class PIC {
  constructor(cpu, master) {
    ...
    if (this.is_master) {
      ...
      this.check_irqs = function () {
        ...
        let enabled_irr = this.irr & this.irq_mask
        ...
        let special_mask = this.special_mask_mode ? this.irq_mask : -1
        ...
        if (PIC_LOG_VERBOSE)
          dbg_log('master> request irq ' + irq_number, LOG_PIC)
      ...
      this.acknowledge_irq = function () {
        ...
        if (this.irr === 0) {
          if (PIC_LOG_VERBOSE)
            dbg_log('master> spurious requested=' + this.requested_irq, LOG_PIC)
          this.requested_irq = -1
        ...
        let irq_mask = 1 << this.requested_irq
      ...
      this.check_irqs = function () {
        ...
        if (PIC_LOG_VERBOSE)
          dbg_log('slave > request irq ' + irq_number, LOG_PIC)
        this.requested_irq = irq_number
      ...
      this.acknowledge_irq = function () {
        ...
        if (this.irr === 0) {
          if (PIC_LOG_VERBOSE)
            dbg_log('slave > spurious requested=' + this.requested_irq, LOG_PIC)
          this.requested_irq = -1
  ...
  set_state(state) {
    ...
    if (this.slave) this.slave.set_state(state[5])
