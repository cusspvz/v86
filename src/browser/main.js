// ;(function () {
//   /** @const */
//   let ON_LOCALHOST = !location.hostname.endsWith('copy.sh')

//   /** @const */
//   let HOST = ON_LOCALHOST ? 'images/' : '//k.copy.sh/'
...
//         ((time / 3600) | 0) +
//         'h ' +
//         v86util.pad0(((time / 60) | 0) % 60, 2) +
//         'm ' +
//         v86util.pad0(time % 60, 2) +
//         's'
//       )
//     }
//   }
...
//       let floppy_file = $('floppy_image').files[0]
//       if (floppy_file) {
//         last_file = floppy_file
//         settings.fda = { buffer: floppy_file }
//       }

//       let cd_file = $('cd_image').files[0]
//       if (cd_file) {
//         last_file = cd_file
//         settings.cdrom = { buffer: cd_file }
//       }

//       let hda_file = $('hda_image').files[0]
//       if (hda_file) {
//         last_file = hda_file
//         settings.hda = { buffer: hda_file }
//       }

//       let hdb_file = $('hdb_image') && $('hdb_image').files[0]
//       if (hdb_file) {
//         last_file = hdb_file
...
//       if ($('multiboot_image')) {
//         let multiboot_file = $('multiboot_image').files[0]
//         if (multiboot_file) {
...
//         hda: {
//           url:
//             HOST + 'reactos-livecd-0.4.15-dev-73-g03c09c9-x86-gcc-lin-dbg.iso',
//           size: 250609664,
//           async: true,
//           use_parts: !ON_LOCALHOST,
//         },
...
//     let query_args = get_query_arguments()
//     let profile = query_args['profile']
...
//         infos.bzimage_initrd_from_filesystem
//       settings.preserve_mac_from_state_image =
//         infos.preserve_mac_from_state_image
...
//     let log_levels = $('log_levels')

//     if (log_levels) {
//       for (let i = 0; i < LOG_NAMES.length; i++) {
//         let mask = LOG_NAMES[i][0]

//         if (mask === 1) continue

//         let name = LOG_NAMES[i][1].toLowerCase(),
...
//     let vga_memory_size = settings.vga_memory_size
...
//     const enable_acpi =
//       settings.acpi === undefined ? $('enable_acpi').checked : settings.acpi

//     /** @const */
//     let BIOSPATH = 'bios/'

//     if (settings.use_bochs_bios) {
//       let biosfile = 'bochs-bios.bin'
//       let vgabiosfile = 'bochs-vgabios.bin'
//     } else {
//       let biosfile = DEBUG ? 'seabios-debug.bin' : 'seabios.bin'
//       let vgabiosfile = DEBUG ? 'vgabios-debug.bin' : 'vgabios.bin'
//     }
...
//       network_relay_url: ON_LOCALHOST
//         ? 'ws://localhost:8080/'
//         : networking_proxy,
...
//         let panel = document.createElement('pre')
...
//       let instruction_counter = emulator.get_instruction_counter()
...
//       let last_ips = instruction_counter - last_instr_counter
...
//           let file = buffer.get_as_file(filename)
...
//     //{
//     //    let memory = emulator.v86.cpu.mem8;
//     //    let memory_size = memory.length;
//     //    let page_size = 4096;
//     //    let header = new Uint8Array(4096);
//     //    let header32 = new Int32Array(header.buffer);
...
//       let was_running = emulator.is_running()
...
//         this.files,
//         function (file) {
//           let loader = new v86util.SyncFileBuffer(file)
...
//           if (uint8array) {
//             let filename = this.value.replace(/\/$/, '').split('/')
...
//     let debug = emulator.v86.cpu.debug
...
//       $('dump_log').value =
//         'Dump log' + (log_data.length ? ' (' + log_data.length + ' lines)' : '')
//     }, 1000)
