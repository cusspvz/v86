const print_stats = {
  stats_to_string: function (cpu) {
    return (
      print_stats.print_misc_stats(cpu) +
  ...
  print_misc_stats: function (cpu) {
    ...
    for (let i = 0; i < stat_names.length; i++) {
      ...
      if (name.includes('/')) {
        ...
        let stat = (stat_values[name] = cpu.wm.exports['profiler_stat_get'](
          i - j
        ))
        value =
          stat >= 100e6
    ...
    text +=
      'do_many_cycles avg: ' +
      (cpu.do_many_cycles_total / cpu.do_many_cycles_count || 0) +
      '\n'
    text +=
  ...
  print_instruction_counts_offset: function (
    cpu,
    compiled,
    jit_exit,
    unguarded_register,
    ...
    for (let i = 0; i < 0x100; i++) {
      text +=
        h(i, 2).slice(2) +
        ':' +
        v86util.pads(Math.round(per_opcode[i] / factor), pad_length)
    ...
    for (let i = 0; i < 0x100; i++) {
      text +=
        h(i & 0xff, 2).slice(2) +
        ':' +
        v86util.pads(Math.round(per_opcode0f[i] / factor), pad_length)
