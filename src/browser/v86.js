...
import { LOG_CPU, WASM_TABLE_OFFSET, WASM_TABLE_SIZE } from '../const'
import { dbg_assert, dbg_log, dbg_trace } from '../log'
import { get_rand_int } from '../utils/get_rand_int'
import { read_sized_string_from_mem } from '../utils/read_sized_string_from_mem'
...
import { SerialAdapter, SerialAdapterXtermJS } from './serial'
import { SpeakerAdapter } from './speaker'
import { SyncBuffer } from '../utils/SyncBuffer'
import { SyncFileBuffer } from '../utils/SyncFileBuffer'
import { AsyncFileBuffer } from '../utils/AsyncFileBuffer'
import { AsyncXHRBuffer, AsyncXHRPartfileBuffer } from '../utils/AsyncXHRBuffer'
import {
  MemoryFileStorage,
 ...
 *
 * Usage: `let emulator = new V86Browser(options);`
 *
 ...
 * @constructor
 */
export function V86Browser(options) {
  //let worker = new Worker("src/browser/worker.js");
  //let adapter_bus = this.bus = WorkerBus.init(worker);
  ...
  const wasm_shared_funcs = {
    ...
    logop: function (eip, op) {
      return cpu.debug.logop(eip, op)
    },
    microtick: microtick,
  ...
  load_file(v86_bin, {
      ...
      WebAssembly.instantiate(bytes, { env: wasm_shared_funcs }).then(
        ...
        (err) => {
          load_file_xhr(v86_bin_fallback, {
            done: (bytes) => {
              WebAssembly.instantiate(bytes, { env: wasm_shared_funcs }).then(
                ({ instance }) => {
                  const imports = wasm_shared_funcs
...
V86Browser.prototype.continue_init = async function (emulator, options) {
  this.bus.register(
    'emulator-stopped',
    function () {
      this.cpu_is_running = false
  ...
  if (options['serial_container_xtermjs']) {
    this.serial_adapter = new SerialAdapterXtermJS(
      options['serial_container_xtermjs'],
      this.bus
    )
  }
  ...
  function add_file(name, file) {
    ...
    if (file.buffer instanceof ArrayBuffer) {
      let buffer = new SyncBuffer(file.buffer)
      files_to_load.push({
        name: name,
        loadable: buffer,
      })
      ...
      let buffer
      if (file.async) {
        buffer = new AsyncFileBuffer(file.buffer)
      } else {
        buffer = new SyncFileBuffer(file.buffer)
      }
      ...
      if (file.async) {
        ...
        if (file.use_parts) {
          buffer = new AsyncXHRPartfileBuffer(file.url, file.size)
        } else {
          buffer = new AsyncXHRBuffer(file.url, file.size)
        }
  ...
  if (options['filesystem']) {
    ...
    let base_url = options['filesystem']['baseurl']
  ...
  let starter = this
  let total = files_to_load.length

  let cont = function (index) {
    if (index === total) {
      setTimeout(done.bind(this), 0)
      return
    }

    let f = files_to_load[index]
  ...
  function done() {
    ...
    if (settings.fs9p && settings.fs9p_json) {
      ...
      if (options['bzimage_initrd_from_filesystem']) {
        const { bzimage, initrd } = this.get_bzimage_initrd_from_filesystem(
          settings.fs9p
        )
...
V86Browser.prototype.get_bzimage_initrd_from_filesystem = function (
  filesystem
) {
 ...
 * @export
 */
V86Browser.prototype.run = function () {
  this.bus.send('cpu-run')
}
 ...
 * @export
 */
V86Browser.prototype.stop = function () {
 ...
 * @export
 */
V86Browser.prototype.destroy = function () {
  this.stop()

  this.v86.destroy()
  if (this.keyboard_adapter) this.keyboard_adapter.destroy()
  if (this.network_adapter) this.network_adapter.destroy()
  if (this.mouse_adapter) this.mouse_adapter.destroy()
  if (this.screen_adapter) this.screen_adapter.destroy()
 ...
 * @export
 */
V86Browser.prototype.restart = function () {
 ...
 * @export
 */
V86Browser.prototype.add_listener = function (event, listener) {
 ...
 * @export
 */
V86Browser.prototype.remove_listener = function (event, listener) {
 ...
 * @export
 */
V86Browser.prototype.restore_state = function (state) {
 ...
 * @export
 */
V86Browser.prototype.save_state = function (callback) {
 ...
 * @export
 */
V86Browser.prototype.get_statistics = function () {
  console.warn(
    'V86Browser.prototype.get_statistics is deprecated. Use events instead.'
  )
 ...
 * @export
 */
V86Browser.prototype.get_instruction_counter = function () {
  if (this.v86) {
 ...
 * @export
 */
V86Browser.prototype.is_running = function () {
 ...
 * @export
 */
V86Browser.prototype.keyboard_send_scancodes = function (codes) {
 ...
 * @export
 */
V86Browser.prototype.keyboard_send_keys = function (codes) {
  for (let i = 0; i < codes.length; i++) {
 ...
 * @export
 */
V86Browser.prototype.keyboard_send_text = function (string) {
 ...
 * @export
 */
V86Browser.prototype.screen_make_screenshot = function () {
 ...
 * @export
 */
V86Browser.prototype.screen_set_scale = function (sx, sy) {
  if (this.screen_adapter) {
 ...
 * @export
 */
V86Browser.prototype.screen_go_fullscreen = function () {
  if (!this.screen_adapter) {
    return
  }

  let elem = document.getElementById('screen_container')
  ...
  if (fn) {
    ...
    let focus_element = document.getElementsByClassName('phone_keyboard')[0]
 ...
 * @export
 */
V86Browser.prototype.lock_mouse = function () {
 ...
 */
V86Browser.prototype.mouse_set_status = function (enabled) {
 ...
 * @export
 */
V86Browser.prototype.keyboard_set_status = function (enabled) {
 ...
 * @export
 */
V86Browser.prototype.serial0_send = function (data) {
 ...
 * @export
 */
V86Browser.prototype.serial_send_bytes = function (serial, data) {
  for (let i = 0; i < data.length; i++) {
 ...
 * @export
 */
V86Browser.prototype.mount_fs = async function (
  path,
  baseurl,
  basefs,
  callback
 ...
 * @export
 */
V86Browser.prototype.create_file = function (file, data, callback) {
  ...
  let path_infos = fs.SearchPath(file)
  let parent_id = path_infos.parentid
  let not_found = filename === '' || parent_id === -1

  if (!not_found) {
    fs.CreateBinaryFile(filename, parent_id, data).then(() => callback(null))
  } else {
    setTimeout(function () {
      callback(new FileNotFoundError())
    }, 0)
 ...
 * @export
 */
V86Browser.prototype.read_file = function (file, callback) {
  let fs = this.fs9p
...
V86Browser.prototype.automatically = function (steps) {
  const run = (steps) => {
    const step = steps[0]
 ...
 * @returns
 */
V86Browser.prototype.read_memory = function (offset, length) {
 ...
 * @param {number} offset
 */
V86Browser.prototype.write_memory = function (blob, offset) {
...
if (typeof window !== 'undefined') {
  window['V86Browser'] = V86Browser
  window['V86'] = V86Browser
} else if (
  typeof module !== 'undefined' &&
  typeof module.exports !== 'undefined'
) {
  module.exports['V86Browser'] = V86Browser
  module.exports['V86'] = V86Browser
} else if (typeof importScripts === 'function') {
  // web worker
  self['V86Browser'] = V86Browser
  self['V86'] = V86Browser
}
