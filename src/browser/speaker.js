/** @const */
let DAC_QUEUE_RESERVE = 0.2

/** @const */
let AUDIOBUFFER_MINIMUM_SAMPLING_RATE = 8000
...
export function SpeakerAdapter(bus) {
  ...
  let SpeakerDAC = window.AudioWorklet
    ? SpeakerWorkletDAC
...
function SpeakerMixer(bus, audio_context) {
  ...
  bus.register(
    ...
    function (data) {
      ...
      let source =
        source_id === MIXER_SRC_MASTER ? this : this.sources.get(source_id)
...
SpeakerMixer.prototype.add_source = function (source_node, source_id) {
  let source = new SpeakerMixerSource(
    this.audio_context,
    source_node,
    this.input_left,
    this.input_right
...
SpeakerMixer.prototype.connect_source = function (source_id, channel) {
  let source = this.sources.get(source_id)
...
SpeakerMixer.prototype.disconnect_source = function (source_id, channel) {
  ...
  if (source === undefined) {
    dbg_assert(
      false,
      'Mixer disconnect - cannot disconnect undefined source: ' + source_id
    )
    return
  }
...
SpeakerMixer.prototype.update = function () {
  let net_gain_left = this.volume_both * this.volume_left * this.gain_left
  let net_gain_right = this.volume_both * this.volume_right * this.gain_right
...
SpeakerMixerSource.prototype.connect = function (channel) {
  let both = !channel || channel === MIXER_CHANNEL_BOTH
...
function PCSpeaker(bus, audio_context, mixer) {
  ...
  bus.register(
    'pcspeaker-update',
    function (data) {
      let counter_mode = data[0]
      let counter_reload = data[1]

      let frequency = 0
      let beep_enabled = counter_mode === 3
...
function SpeakerWorkletDAC(bus, audio_context, mixer) {
  ...
  function worklet() {
    /** @const */
    let RENDER_QUANTUM = 128

    /** @const */
    let MINIMUM_BUFFER_SIZE = 2 * RENDER_QUANTUM

    /** @const */
    let QUEUE_RESERVE = 1024
    ...
    function DACProcessor() {
      let self = Reflect.construct(AudioWorkletProcessor, [], DACProcessor)
    ...
    DACProcessor.prototype['process'] = DACProcessor.prototype.process =
      function (inputs, outputs, parameters) {
        for (let i = 0; i < outputs[0][0].length; i++) {
          ...
          let start = this.source_offset - this.kernel_size + 1
          let end = this.source_offset + this.kernel_size

          for (let j = start; j <= end; j++) {
            let convolute_index = this.source_block_start + j
            sum0 +=
    ...
    DACProcessor.prototype.ensure_enough_data = function (needed) {
      let current_length = this.source_buffer_current[0].length
      let remaining = current_length - this.source_block_start
    ...
    DACProcessor.prototype.prepare_next_buffer = function () {
      ...
      if (sample_count < MINIMUM_BUFFER_SIZE) {
        ...
        let new_big_buffer_size = Math.max(sample_count, MINIMUM_BUFFER_SIZE)
        let new_big_buffer = [
        ...
        let new_big_buffer_pos = this.source_buffer_current[0].length

        // Copy the rest.
        for (let i = 0; i < buffer_count; i++) {
          let small_buffer = this.queue_shift()
    ...
    DACProcessor.prototype.queue_shift = function () {
      ...
      let item = this.queue_data[this.queue_start]
  ...
  let worklet_string = worklet.toString()

  let worklet_code_start = worklet_string.indexOf('{') + 1
  let worklet_code_end = worklet_string.lastIndexOf('}')
  ...
  if (DEBUG) {
    worklet_code = 'let DEBUG = true;\n' + worklet_code
  }

  let worklet_blob = new Blob([worklet_code], {
    type: 'application/javascript',
  })
  let worklet_url = URL.createObjectURL(worklet_blob)
...
SpeakerBufferSourceDAC.prototype.queue = function (data) {
  ...
  let sample_count = data[0].length
  let block_duration = sample_count / this.sampling_rate

  let buffer
  if (this.rate_ratio > 1) {
    let new_sample_count = sample_count * this.rate_ratio
    let new_sampling_rate = this.sampling_rate * this.rate_ratio
    buffer = this.audio_context.createBuffer(
      2,
      new_sample_count,
      new_sampling_rate
    )
    let buffer_data0 = buffer.getChannelData(0)
    let buffer_data1 = buffer.getChannelData(1)
  ...
  let current_time = this.audio_context.currentTime
  ...
  if (this.buffered_time < current_time) {
    ...
    let target_silence_duration = DAC_QUEUE_RESERVE - block_duration
...
SpeakerDACDebugger.prototype.download_txt = function (history_id, channel) {
  let txt = this.output_history[history_id][channel]
    .map((v) => v.join(' '))
    .join(' ')
...
SpeakerDACDebugger.prototype.download_csv = function (history_id) {
  let buffers = this.output_history[history_id]
  let csv_rows = []
