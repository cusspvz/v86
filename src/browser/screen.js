...
export function ScreenAdapter(screen_container, bus) {
   ...
   * @const
   */
  let charmap_high = new Uint16Array([
  ...
  /** @const */
  let charmap_low = new Uint16Array([
  ...
  this.set_size_graphical = function (
    ...
    graphic_image_data = graphic_context.createImageData(
      buffer_width,
      buffer_height
    )
  ...
  this.text_update_row = function (row) {
    let offset = 3 * row * text_mode_width,
      row_element,
      color_element,
      fragment

    let bg_color, fg_color, text
    ...
    for (let i = 0; i < text_mode_width; ) {
      ...
      while (
        ...
        let ascii = text_mode_data[offset]
