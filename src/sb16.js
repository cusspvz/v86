...
/** @const */ export const DSP_COPYRIGHT =
...
// as a command number.
/** @const */ export const DSP_NO_COMMAND = 0
// Size (bytes) of the DSP write/read buffers
/** @const */ export const DSP_BUFSIZE = 64
// Size (bytes) of the buffers containing floating point linear PCM audio.
/** @const */ export const DSP_DACSIZE = 65536
// Size (bytes) of the buffer in which DMA transfers are temporarily
// stored before being processed.
/** @const */ export const SB_DMA_BUFSIZE = 65536
// Number of samples to attempt to retrieve per transfer.
/** @const */ export const SB_DMA_BLOCK_SAMPLES = 1024
// Usable DMA channels.
/** @const */ export const SB_DMA0 = 0
/** @const */ export const SB_DMA1 = 1
/** @const */ export const SB_DMA3 = 3
/** @const */ export const SB_DMA5 = 5
/** @const */ export const SB_DMA6 = 6
/** @const */ export const SB_DMA7 = 7
// Default DMA channels.
/** @const */ export const SB_DMA_CHANNEL_8BIT = SB_DMA1
/** @const */ export const SB_DMA_CHANNEL_16BIT = SB_DMA5
// Usable IRQ channels.
/** @const */ export const SB_IRQ2 = 2
/** @const */ export const SB_IRQ5 = 5
/** @const */ export const SB_IRQ7 = 7
/** @const */ export const SB_IRQ10 = 10
// Default IRQ channel.
/** @const */ export const SB_IRQ = SB_IRQ5
// Indices to the irq_triggered register.
/** @const */ export const SB_IRQ_8BIT = 0x1
/** @const */ export const SB_IRQ_16BIT = 0x2
/** @const */ export const SB_IRQ_MIDI = 0x1
/** @const */ export const SB_IRQ_MPU = 0x4

// Probably less efficient, but it's more maintainable, instead
// of having a single large unorganised and decoupled table.
let DSP_COMMAND_SIZES = new Uint8Array(256)
let DSP_COMMAND_HANDLERS = []
let MIXER_READ_HANDLERS = []
let MIXER_WRITE_HANDLERS = []
let MIXER_REGISTER_IS_LEGACY = new Uint8Array(256)
let FM_HANDLERS = []
...
SB16.prototype.port2xE_read = function () {
  ...
  let ready = this.read_buffer.length && !this.dsp_highspeed
  return (ready << 7) | 0x7f
}
...
register_dsp_command([0x10], 1, function () {
  let value = audio_normalize(this.write_buffer.shift(), 127.5, -1)
...
register_dsp_command(any_first_digit(0xb0), 3, function () {
  ...
  let mode = this.write_buffer.shift()
...
let SB_F9 = new Uint8Array(256)
SB_F9[0x0e] = 0xff
SB_F9[0x0f] = 0x07
SB_F9[0x37] = 0x38
register_dsp_command([0xf9], 1, function () {
  let input = this.write_buffer.shift()
...
SB16.prototype.mixer_read = function (address) {
  let handler = MIXER_READ_HANDLERS[address]
  let data
  if (handler) {
    ...
    dbg_log(
      'unhandled mixer register read. addr:' + h(address) + ' data:' + h(data),
      LOG_SB16
    )
  }
  return data
...
SB16.prototype.mixer_write = function (address, data) {
  let handler = MIXER_WRITE_HANDLERS[address]
  if (handler) {
    handler.call(this, data)
  } else {
    dbg_log(
      'unhandled mixer register write. addr:' + h(address) + ' data:' + h(data),
      LOG_SB16
    )
  }
}
...
function register_mixer_legacy(
  ...
  MIXER_READ_HANDLERS[address_old] = function () {
    let left = this.mixer_registers[address_new_left] & 0xf0
    let right = this.mixer_registers[address_new_right] >>> 4
    return left | right
  }
  ...
  MIXER_WRITE_HANDLERS[address_old] = function (data) {
    ...
    let prev_left = this.mixer_registers[address_new_left]
    let prev_right = this.mixer_registers[address_new_right]
    let left = (data & 0xf0) | (prev_left & 0x0f)
    let right = ((data << 4) & 0xf0) | (prev_right & 0x0f)
...
//    let prev = this.mixer_registers[0x3A];
...
/** @const */ let SB_FM_OPERATORS_BY_OFFSET = new Uint8Array(32)
...
register_fm_write(between(0x20, 0x35), function (bits, register, address) {
  // eslint-disable-next-line no-unused-vars
  let operator = get_fm_operator(register, address - 0x20)
  // Tremolo
  // Vibrato
  // Sustain
...
register_fm_write(between(0x40, 0x55), function (bits, register, address) {
  // eslint-disable-next-line no-unused-vars
  let operator = get_fm_operator(register, address - 0x40)
  // Key Scale Level
  // Output Level
})
...
register_fm_write(between(0x60, 0x75), function (bits, register, address) {
  // eslint-disable-next-line no-unused-vars
  let operator = get_fm_operator(register, address - 0x60)
  // Attack Rate
  // Decay Rate
})
...
register_fm_write(between(0x80, 0x95), function (bits, register, address) {
  // eslint-disable-next-line no-unused-vars
  let operator = get_fm_operator(register, address - 0x80)
  // Sustain Level
  // Release Rate
})
...
register_fm_write(between(0xe0, 0xf5), function (bits, register, address) {
  // eslint-disable-next-line no-unused-vars
  let operator = get_fm_operator(register, address - 0xe0)
  // Waveform Select
})
...
SB16.prototype.dma_transfer_start = function () {
  ...
  let max_bytes_block = Math.max((this.dma_bytes_count >> 2) & ~0x3, 32)
...
SB16.prototype.dma_transfer_next = function () {
  ...
  let size = Math.min(this.dma_bytes_left, this.dma_bytes_block)
  let samples = Math.floor(size / this.bytes_per_sample)
...
SB16.prototype.dma_to_dac = function (sample_count) {
  let amplitude = this.dsp_16bit ? 32767.5 : 127.5
  let offset = this.dsp_signed ? 0 : -1
  let repeats = this.dsp_stereo ? 1 : 2
  ...
  for (let i = 0; i < sample_count; i++) {
    let sample = audio_normalize(buffer[i], amplitude, offset)
...
SB16.prototype.dac_send = function () {
  ...
  let out0 = this.dac_buffers[0].shift_block(this.dac_buffers[0].length)
  let out1 = this.dac_buffers[1].shift_block(this.dac_buffers[1].length)
