import { LOG_HPET } from './const'
import { dbg_log } from './log'
import { h } from './utils/h'

export const HPET_ADDR = 0xfed00000
export const HPET_PERIOD = 0x05f5e100 // in nano seconds
export const HPET_FREQ_MS = 1e12 / HPET_PERIOD // in kHZ
export const HPET_SUPPORT_64 = 0
export const HPET_COUNTER_CONFIG = (1 << 4) | (HPET_SUPPORT_64 << 5)
export const HPET_COUNTER_CONFIG_MASK = (1 << 4) | (1 << 5) | (1 << 15)
export const HPET_NUM_COUNTERS = 4
...
export function HPET(cpu) {
  ...
  //let counter_last_irq = new Int32Array(HPET_NUM_COUNTERS << 1);
  ...
  this.timer = function (now) {
    ...
    var counter_value = get_counter() >>> 0,
      config,
      //last_irq,
      comparator,
      do_irq

    for (let i = 0; i < HPET_NUM_COUNTERS; i++) {
  ...
  function get_counter_high() {
    if (HPET_SUPPORT_64) {
      if (hpet_enabled) {
        return (
          ((Date.now() - hpet_start) * (HPET_FREQ_MS / 0x100000000) +
            hpet_offset_high) |
          0
        )
      } else {
  ...
  function mmio_read(addr) {
    dbg_log(
      'Read ' + h(addr, 4) + ' (ctr=' + h(get_counter() >>> 0) + ')',
      LOG_HPET
    )
    ...
    switch (register) {
      case 0:
        return (
          (counter_config[counter << 1] & ~HPET_COUNTER_CONFIG_MASK) |
          HPET_COUNTER_CONFIG
        )
      case 1:
  ...
  function mmio_write(addr, data) {
    ...
    switch (addr) {
      case 0x10:
        dbg_log(
          'conf: enabled=' + (data & 1) + ' legacy=' + ((data >> 1) & 1),
          LOG_HPET
        )
    ...
    if (addr < 0x100 || counter >= HPET_NUM_COUNTERS || register > 2) {
      dbg_log(
        'Write reserved address: ' + h(addr) + ' data=' + h(data),
        LOG_HPET
      )
      return
    }
    ...
    switch (register) {
      ...
      case 2:
        if (counter_read_acc_next) {
          ...
          counter_read_acc_next = false
          dbg_log(
            'Accumulator acc=' + h(data >>> 0, 8) + ' ctr=' + h(counter, 2),
            LOG_HPET
          )
        } else {
