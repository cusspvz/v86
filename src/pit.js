 ...
 * In kHz
 */
let OSCILLATOR_FREQ = 1193.1816666 // 1.193182 MHz
...
export class PIT {
  constructor(cpu, bus) {
    ...
    cpu.io.register_read(0x61, this, function () {
      ...
      let ref_toggle = (now * ((1000 * 1000) / 15000)) & 1
      let counter2_out = this.did_rollover(2, now)
  ...
  get_counter_value(i, now) {
    if (!this.counter_enabled[i]) {
      return 0
    }

    let diff = now - this.counter_start_time[i]
    let diff_in_ticks = Math.floor(diff * OSCILLATOR_FREQ)
    ...
    let reload = this.counter_reload[i]
  ...
  counter_read(i) {
    let latch = this.counter_latch[i]
    ...
    if (latch) {
      ...
      let next_low = this.counter_next_low[i]

      if (this.counter_mode[i] === 3) {
        this.counter_next_low[i] ^= 1
      }

      let value = this.get_counter_value(i, microtick())
  ...
  counter_write(i, value) {
    ...
    if (this.counter_read_mode[i] !== 3 || !this.counter_next_low[i]) {
      ...
      this.counter_start_time[i] = microtick()
